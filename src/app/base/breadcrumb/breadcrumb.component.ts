...
export class BreadcrumbComponent implements OnInit {
  ...
  private setPageTitle(url: string): void {
    const currentPage: SidenavItem = this.sidenavItems.find(item => '/' + item.id === url);
    if (currentPage) {
      this.pageTitle = currentPage.labels[this.lang];
    } else {
      this.pageTitle = 'Page Title'; // Définissez un titre par défaut si nécessaire
    }
  }
}